...
1 feature を **N 個の独立 sub-PR に分けて並列実装** する標準構成 (過去の大規模移行で
確立した運用パターン)。
...
- **各 worker pane = 専用 worktree + 専用 branch**: `.worktrees/<topic>`
  (例: `.worktrees/<feature>-a` / `<feature>-b`) を切り、worker は起動直後に
  `cd .worktrees/<topic>` で自 worktree へ移動して作業

#### なぜ worktree 分離が必須か

cmux pane は cwd を共有するので、worker が共有 dir で `git checkout -b` した
瞬間 **全 pane の working tree が切替わる**。過去 dogfood で 2 worker + coordinator
が同一 dir を共有 → worker1 の checkout が他 pane を巻き込む事故を、worker 専用
worktree で構造的に解消した (上記「Worktree 分離」の事故と同 class)。

#### worktree 作成 (coordinator が bootstrap 時に)

```bash
# 各 worker 用に専用 worktree + 専用 branch を切る (-b 必須、origin/main を base)
git worktree add -b feat/<topic>-a  .worktrees/<feature>-a  origin/main
git worktree add -b feat/<topic>-b  .worktrees/<feature>-b  origin/main
```
